 ...
 */
var dangerouslyProcessChildrenUpdates = function(
  childrenUpdates,
  markupList,
) {
  ...
  for (var i = 0; i < childrenUpdates.length; i++) {
    ...
    var containerTag = ReactNativeTagHandles.mostRecentMountedNodeHandleForRootNodeID(
      update.parentID,
    );
    var updates =
      byContainerTag[containerTag] || (byContainerTag[containerTag] = {});
    if (update.type === ReactMultiChildUpdateTypes.MOVE_EXISTING) {
      (updates.moveFromIndices || (updates.moveFromIndices = [])).push(
        update.fromIndex,
      );
      (updates.moveToIndices || (updates.moveToIndices = [])).push(
        update.toIndex,
      );
    } else if (update.type === ReactMultiChildUpdateTypes.REMOVE_NODE) {
      (updates.removeAtIndices || (updates.removeAtIndices = [])).push(
        update.fromIndex,
      );
      ...
      (updates.addAtIndices || (updates.addAtIndices = [])).push(
        update.toIndex,
      );
...
var ReactNativeDOMIDOperations = {
  ...
  dangerouslyReplaceNodeWithMarkupByID: ReactPerf.measure(
    ...
    function(id, mountImage) {
      var oldTag = ReactNativeTagHandles.mostRecentMountedNodeHandleForRootNodeID(
        id,
      );
