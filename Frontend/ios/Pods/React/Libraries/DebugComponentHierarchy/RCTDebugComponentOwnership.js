...
var DebugComponentOwnershipModule = require('NativeModules')
...
function getRootTagForTag(tag: number): ?number {
  ...
  var rootID = ReactNativeTagHandles.getNativeTopRootIDFromNodeID(
    rootNodeID,
  );
  if (!rootID) {
    return null;
...
module.exports = {
  ...
  getOwnerHierarchy: function(requestID: number, tag: number) {
    ...
    var ownerHierarchy = instance
      ...
      : null;
    DebugComponentOwnershipModule.receiveOwnershipHierarchy(
      requestID,
      tag,
      ownerHierarchy,
    );
