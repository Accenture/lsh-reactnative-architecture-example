...
var JSTimersExecution = {
  ...
  callTimer: function(timerID) {
    warning(
      ...
      'Tried to call timer with ID ' +
        timerID +
        ' but no such timer exists',
    );
    ...
    // Clear the metadata
    if (
      type === JSTimersExecution.Type.setTimeout ||
    ...
    try {
      if (
        ...
        console.error(
          'Tried to call a callback with invalid type: ' + type,
        );
        return;
      }
    } catch (e) {
  ...
  callTimers: function(timerIDs) {
    invariant(
      timerIDs.length !== 0,
      'Probably shouldn\'t call "callTimers" with no timerIDs',
    );
