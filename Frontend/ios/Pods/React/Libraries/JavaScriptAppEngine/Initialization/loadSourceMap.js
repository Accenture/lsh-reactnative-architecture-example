...
function loadSourceMap(): Promise {
  return fetchSourceMap().then(map => new SourceMapConsumer(map));
}
...
function fetchSourceMap(): Promise {
  ...
  if (!RCTSourceCode) {
    return Promise.reject(
      new Error('RCTSourceCode module is not available'),
    );
  }
  ...
  if (!RCTNetworking) {
    ...
    return Promise.reject(
      new Error('RCTNetworking module is not available'),
    );
  }
  ...
  return new Promise(RCTSourceCode.getScriptText)
    ...
    .then(url => {
      if (url === null) {
        return Promise.reject(
          new Error(
            'No source map URL found. May be running from bundled file.',
          ),
        );
      }
