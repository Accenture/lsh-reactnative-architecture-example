...
function reportException(e: Exception, isFatal: boolean, stack?: any) {
  ...
  if (RCTExceptionsManager) {
    ...
    if (isFatal) {
      RCTExceptionsManager.reportFatalException(
        e.message,
        stack,
        currentExceptionID,
      );
...
function stackToString(stack) {
  var maxLength = Math.max.apply(
    null,
    stack.map(frame => frame.methodName.length),
  );
  return stack
    .map(frame => stackFrameToString(frame, maxLength))
    .join('\n');
}
