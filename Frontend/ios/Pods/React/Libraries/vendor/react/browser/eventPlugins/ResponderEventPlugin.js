...
function setResponderAndExtractTransfer(
  ...
  var wantsResponderID = executeDispatchesInOrderStopAtTrue(
    shouldSetEvent,
  );
...
function canTriggerTransfer(topLevelType, topLevelTargetID, nativeEvent) {
  return (
    ...
    ((topLevelType === EventConstants.topLevelTypes.topScroll &&
      ...
      (trackedTouchCount > 0 &&
        topLevelType ===
          EventConstants.topLevelTypes.topSelectionChange) ||
...
function noResponderTouches(nativeEvent) {
  ...
  for (var i = 0; i < touches.length; i++) {
    ...
    if (target !== null && target !== undefined && target !== 0) {
      ...
      var isAncestor = ReactInstanceHandles.isAncestorIDOf(
        responderID,
