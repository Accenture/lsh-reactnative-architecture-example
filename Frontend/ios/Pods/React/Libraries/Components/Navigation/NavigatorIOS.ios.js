...
var NavigatorIOS = React.createClass({
  ...
  componentWillMount: function() {
    ...
    this._emitWillFocus(
      this.state.routeStack[this.state.observedTopOfStack],
    );
  },
  ...
  _eliminateUnneededChildren: function() {
    ...
    var updatingAllIndicesAtOrBeyond =
      this.state.routeStack.length > this.state.observedTopOfStack + 1
        ...
        : null;
    this.setState({
      idStack: this.state.idStack.slice(
        0,
        this.state.observedTopOfStack + 1,
      ),
    ...
    if (this.state.requestedTopOfStack === this.state.observedTopOfStack) {
      if (this.state.requestedTopOfStack > 0) {
        this._tryLockNavigator(() => {
          ...
          this._emitWillFocus(
            this.state.routeStack[newRequestedTopOfStack],
          );
          this.setState({
  ...
  handleNavigationComplete: function(e: Event) {
    if (this._toFocusOnNavigationComplete) {
      this._getFocusEmitter().emit(
        'focus',
        this._toFocusOnNavigationComplete,
      );
    ...
    return (
      <StaticContainer key={'nav' + i} shouldUpdate={shouldUpdateChild}>
        <RCTNavigatorItem
          ...
          rightButtonIcon={this._imageNameFromSource(
