...
describe('deepDiffer', function() {
  ...
  it('should diff Objects', () => {
    ...
    expect(deepDiffer({ a: { A: 1 }, b: 1 }, { a: { A: 1 }, b: 1 })).toBe(
      false,
    );
    expect(deepDiffer({ a: { A: 1 }, b: 1 }, { a: { A: 2 }, b: 1 })).toBe(
      true,
    );
    expect(
      deepDiffer(
  ...
  it('should diff Arrays', () => {
    ...
    expect(deepDiffer([0, null, false, ''], [0, null, false, ''])).toBe(
      false,
    );
    expect(deepDiffer([0, null, false, ''], ['', false, null, 0])).toBe(
      true,
    );
  });
  ...
  it('should distinguish between proper Array and Object', () => {
    expect(deepDiffer(['a', 'b'], { 0: 'a', 1: 'b', length: 2 })).toBe(
      true,
    );
    expect(deepDiffer(['a', 'b'], { length: 2, 0: 'a', 1: 'b' })).toBe(
      true,
    );
  });
