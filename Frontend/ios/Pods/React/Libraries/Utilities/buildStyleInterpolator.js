...
var inline = function(func, replaceWithArgs) {
  ...
  var replaceRegexStr = parameterNames
    .map(function(paramName) {
      return '\\b' + paramName + '\\b';
    })
    .join('|');
  var replaceRegex = new RegExp(replaceRegexStr, 'g');
  var fnBody = fnStr.substring(
    fnStr.indexOf('{') + 1,
    fnStr.lastIndexOf('}'),
  );
...
var MatrixOps = {
  ...
  transformTranslate: function(matVar, opVar) {
    ...
    var z = opVar[2];
    matVar[12] =
      matVar[0] * x + matVar[4] * y + matVar[8] * z + matVar[12];
    matVar[13] =
      matVar[1] * x + matVar[5] * y + matVar[9] * z + matVar[13];
    matVar[14] =
      matVar[2] * x + matVar[6] * y + matVar[10] * z + matVar[14];
    matVar[15] =
      matVar[3] * x + matVar[7] * y + matVar[11] * z + matVar[15];
  },
 ...
 */
var computeNextMatrixOperationField = function(
  anim,
  name,
  dimension,
  index,
  ...
  if (
    anim.from[dimension] !== undefined &&
    anim.to[dimension] !== undefined
  ) {
    return '  ' + anim.from[dimension] !== anim.to[dimension]
      ? computeNextValLinear(
          anim,
          anim.from[dimension],
          anim.to[dimension],
          fieldAccess,
    ...
    return (
      '  ' + fieldAccess + ' = ' + InitialOperationField[name][index] + ';'
    );
  }
};
...
var setNextMatrixAndDetectChange = function(orderedMatrixOperations) {
  ...
  for (var i = 0; i < orderedMatrixOperations.length; i++) {
    ...
    if (i === 0) {
      fn.push.apply(
        fn,
        inline(MatrixOpsInitial[opName], [
          'transformMatrix',
          operationVar(opName),
        ]),
      );
  ...
  fn.push.apply(
    fn,
    inline(
      MatrixOps.matrixDiffers,
      ['didChange', 'transformMatrix'].concat(unrolledVars),
    ),
  );
  return fn;
};
    ...
    if (anim.type === 'linear') {
      if (InterpolateMatrix[name]) {
        ...
        if (name === TRANSFORM_ROTATE_NAME) {
          setOperations.push(
            computeNextMatrixOperationField(anim, name, W_DIM, 3),
          );
        }
