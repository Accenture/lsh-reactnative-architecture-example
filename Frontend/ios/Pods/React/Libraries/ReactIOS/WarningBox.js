...
if (__DEV__) {
  console.warn = function() {
    ...
    var warning = Array.prototype.map
      .call(arguments, stringifySafe)
      .join(' ');
    if (
      !console.yellowBoxResetIgnored &&
      ignoredWarnings.indexOf(warning) !== -1
    ) {
      return;
    }
    var count = warningCounts.has(warning)
      ? warningCounts.get(warning) + 1
      : 1;
...
var WarningRow = React.createClass({
  componentWillMount: function() {
    ...
    this.dismissalSpring.addListener({
      onSpringUpdate: () => {
        ...
        this.closeButton &&
          this.closeButton.setNativeProps({
            opacity: 1 - val * 5,
          });
      },
      onSpringAtRest: () => {
    ...
    this.panGesture = PanResponder.create({
      ...
      onPanResponderMove: (e, gestureState) => {
        ...
        this.dismissalSpring.setCurrentValue(
          gestureState.dx / SCREEN_WIDTH,
        );
      },
  ...
  render: function() {
    ...
    return (
      ...
      >
        <TouchableOpacity onPress={this.props.onOpened}>
          <Text
    ...
    return (
      ...
      >
        ...
        <View style={styles.yellowBoxButtons}>
          ...
          >
            <Text style={styles.yellowBoxButtonText}>Dismiss</Text>
          </TouchableOpacity>
          <TouchableOpacity
            onPress={this.props.onIgnored}
            style={styles.yellowBoxButton}
          >
            <Text style={styles.yellowBoxButtonText}>Ignore</Text>
          </TouchableOpacity>
        </View>
      </TouchableOpacity>
    );
...
var WarningBox = React.createClass({
  ...
  render: function() {
    ...
    return <View style={styles.warningContainer}>{warningRows}</View>;
  },
});
