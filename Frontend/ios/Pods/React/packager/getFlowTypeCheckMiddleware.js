    ...
    exec('command -v flow >/dev/null 2>&1', function(error, stdout) {
      if (error) {
        if (!hasWarned.noFlow) {
          ...
          console.warn(
            chalk.yellow(
              'flow: Skipping because not installed.  Install with ' +
                '`brew install flow`.',
            ),
          );
        }
    ...
    if (!flowError) {
      ...
      try {
        ...
        var error = {
          status: 200,
          message:
            'Flow found type errors.  If you think these are wrong, ' +
...
function _endFlowBad(res) {
  ...
  _endRes(
    res,
    'Flow failed to run! Please look at the console for more details.',
    '200',
    false,
  );
}
