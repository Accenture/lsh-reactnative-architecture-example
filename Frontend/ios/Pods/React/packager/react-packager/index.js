...
// compat.
exports.buildPackage = exports.buildBundle = function(
  options,
  bundleOptions,
) {
  var server = createServer(options);
  return server.buildBundle(bundleOptions).then(function(p) {
    server.end();
    return p;
  });
};

exports.buildPackageFromUrl = exports.buildBundleFromUrl = function(
  options,
  reqUrl,
) {
  var server = createServer(options);
  return server.buildBundleFromUrl(reqUrl).then(function(p) {
    server.end();
    return p;
  });
};
...
exports.getDependencies = function(options, main) {
  var server = createServer(options);
  return server.getDependencies(main).then(function(r) {
    server.end();
