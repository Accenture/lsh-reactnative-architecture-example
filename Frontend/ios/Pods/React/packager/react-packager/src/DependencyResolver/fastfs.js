...
class Fastfs extends EventEmitter {
  ...
  build() {
    ...
    return this._crawling.then(files => {
      const fastfsActivity = Activity.startEvent(
  ...
  closest(filePath, name) {
    for (
      let file = this._getFile(filePath).parent;
      file;
      file = file.parent
    ) {
  ...
  _getAndAssertRoot(filePath) {
    ...
    if (!root) {
      const error = new Error(
        `File ${filePath} not found in any of the roots`,
      );
  ...
  _getFile(filePath) {
    ...
    if (!hasOwn.call(this._fastPaths, filePath)) {
      this._fastPaths[filePath] = this._getAndAssertRoot(
        filePath,
...
class File {
  ...
  addChild(file) {
    ...
    if (parts.length === 1) {
      ...
      const dir = new File(path.join(this.path, parts[0]), {
        isDir: true,
      });
      dir.parent = this;
  ...
  getFiles() {
    return _.flatten(
      _.values(this.children).map(file => {
        if (file.isDir) {
