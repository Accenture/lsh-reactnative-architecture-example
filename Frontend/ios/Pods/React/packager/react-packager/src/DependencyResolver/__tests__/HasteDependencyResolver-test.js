...
describe('HasteDependencyResolver', function() {
  ...
  describe('getDependencies', function() {
    pit('should get dependencies with polyfills', function() {
      ...
      return depResolver
        .getDependencies('/root/index.js', { dev: false })
        .then(function(result) {
          expect(result.mainModuleId).toEqual('index');
          expect(result.dependencies[result.dependencies.length - 1]).toBe(
            module,
          );
    ...
    pit('should get dependencies with polyfills', function() {
      ...
      return depResolver
        .getDependencies('/root/index.js', { dev: true })
        .then(function(result) {
          expect(result.mainModuleId).toEqual('index');
          expect(depGraph.getDependencies).toBeCalledWith(
            '/root/index.js',
            undefined,
          );
    ...
    pit('should pass in more polyfills', function() {
      ...
      return depResolver
        ...
        .then(result => {
          expect(result.mainModuleId).toEqual('index');
          expect(
            Polyfill.mock.calls[result.dependencies.length - 2],
          ).toEqual([
            {
              path: 'some module',
              id: 'some module',
  ...
  describe('wrapModule', function() {
    pit('should resolve modules', function() {
      ...
      resolutionResponse.getResolvedDependencyPairs = module => {
        return [['x', createModule('changed')], ['y', createModule('Y')]];
      };
