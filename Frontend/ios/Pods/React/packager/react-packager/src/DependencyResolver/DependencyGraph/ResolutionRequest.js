...
class ResolutionRequest {
  ...
  resolveDependency(fromModule, toModuleName) {
    ...
    return this._resolveNodeDependency(fromModule, toModuleName).then(
      cacheResult,
      forgive,
    );
  }
  ...
  getAsyncDependencies(response) {
    return Promise.resolve()
        ...
        return mod
          ...
          .then(bundles =>
            Promise.all(
              bundles.map(bundle =>
                Promise.all(
                  bundle.map(dep => this.resolveDependency(mod, dep)),
                ),
              ),
      ...
      .then(asyncDependencies =>
        asyncDependencies.forEach(dependency =>
          response.pushAsyncDependency(dependency),
        ),
      );
  }
      ...
      return this._redirectRequire(fromModule, toModuleName).then(
        realModuleName => {
          ...
          let p = Promise.reject(
            new UnableToResolveError('Node module not found'),
          );
  ...
  _loadAsFile(potentialModulePath) {
    return Promise.resolve().then(() => {
      ...
      if (this._fastfs.fileExists(potentialModulePath)) {
        ...
        this._platform != null &&
        this._fastfs.fileExists(
          potentialModulePath + '.' + this._platform + '.js',
        )
      ) {
