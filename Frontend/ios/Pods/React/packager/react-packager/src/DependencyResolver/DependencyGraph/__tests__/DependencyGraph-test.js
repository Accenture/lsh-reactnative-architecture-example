...
describe('DependencyGraph', function() {
  ...
  function getOrderedDependenciesAsJSON(dgraph, entry, platform) {
    return dgraph
      .getDependencies(entry, platform)
      .then(response => response.finalize())
      .then(({ dependencies }) =>
        Promise.all(
          dependencies.map(dep =>
            Promise.all([dep.getName(), dep.getDependencies()]).then(
  ...
  describe('get sync dependencies', function() {
    ...
    pit(
      'should resolve require to main if it is a dir w/ a package.json',
      function() {
        var root = '/root';
        fs.__setMockFilesystem({
          root: {
  ...
  describe('node_modules', function() {
    ...
    pit(
      'should selectively ignore providesModule in node_modules',
      function() {
        var root = '/root';
        fs.__setMockFilesystem({
          root: {
          ...
          function(deps) {
            expect(deps).toEqual([
              ...
              },
              {
                ...
                path:
                  '/root/node_modules/react-tools/node_modules/submodule/main.js',
                dependencies: [],
                isAsset: false,
                isAsset_DEPRECATED: false,
                isJSON: false,
    ...
    pit(
      'should not be confused by prev occuring whitelisted names',
      function() {
    ...
    pit(
      'should ignore modules it cant find (assumes own require system)',
      function() {
    ...
    pit(
      'should work with node packages with a .js in the name',
      function() {
        var root = '/root';
        fs.__setMockFilesystem({
          root: {
  ...
  describe('file watch updating', function() {
    ...
    pit('updates module dependencies', function() {
      ...
      return getOrderedDependenciesAsJSON(dgraph, '/root/index.js').then(
        function() {
          filesystem.root['index.js'] = filesystem.root[
            'index.js'
          ].replace('require("foo")', '');
    ...
    pit('should update node package main changes', function() {
      ...
      return getOrderedDependenciesAsJSON(dgraph, '/root/index.js').then(
        function(deps) {
          filesystem.root.node_modules.foo[
            'package.json'
          ] = JSON.stringify({
            name: 'foo',
            main: 'main.js',
  ...
  describe('getAsyncDependencies', () => {
    pit('should get dependencies', function() {
      ...
      return dgraph
        .getDependencies('/root/index.js')
        .then(response => response.finalize())
        .then(({ asyncDependencies }) => {
          expect(asyncDependencies).toEqual([['/root/a.js']]);
        });
    });
  });
});
