...
function watchmanRecReadDir(roots, { ignore, fileWatcher, exts }) {
  const files = [];
  return Promise.all(
    roots.map(root => fileWatcher.getWatcherForRoot(root)),
  )
    .then(watchers => {
      ...
      const uniqueWatchers = watchers.filter(
        (watcher, i) =>
          watchmanRoots.indexOf(watcher.watchProjectInfo.root) === i,
      );
      ...
      return Promise.all(
        uniqueWatchers.map(watcher => {
          ...
          const cmd = Promise.denodeify(
            watcher.client.command.bind(watcher.client),
          );
          return cmd([
            'query',
            watchedRoot,
            ...
            resp.files.forEach(filePath => {
              filePath = path.join(watchedRoot, filePath);
