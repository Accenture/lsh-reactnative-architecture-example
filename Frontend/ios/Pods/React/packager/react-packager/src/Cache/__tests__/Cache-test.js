...
describe('JSTransformer Cache', () => {
  ...
  describe('getting/setting', () => {
    pit('calls loader callback for uncached file', () => {
      ...
      return cache
        ...
        .then($ => expect(loaderCb).toBeCalledWith('/rootDir/someFile'));
    });
    ...
    pit('supports storing multiple fields', () => {
      ...
      var loaderCb = jest
        .genMockFn()
        .mockImpl(() => Promise.resolve(index++));
    ...
    pit('caches the value after the first call', () => {
      ...
      return cache.get('/rootDir/someFile', 'field', loaderCb).then(() => {
        var shouldNotBeCalled = jest.genMockFn();
        return cache
          .get('/rootDir/someFile', 'field', shouldNotBeCalled)
          .then(value => {
    ...
    pit(
      'clears old field when getting new field and mtime changed',
      () => {
        var mtime = 0;
        require('fs').stat.mockImpl((file, callback) => {
          callback(null, {
        ...
        var loaderCb = jest
          .genMockFn()
          .mockImpl(() => Promise.resolve('lol' + mtime));
  ...
  describe('loading cache from disk', () => {
    ...
    pit('should load cache from disk', () => {
      ...
      return cache
        ...
        .then(value => {
          ...
          return cache.get('/rootDir/foo', 'field', loaderCb).then(val => {
            expect(loaderCb).not.toBeCalled();
    ...
    pit('should not load outdated cache', () => {
      ...
      var loaderCb = jest
        .genMockFn()
        .mockImpl(() => Promise.resolve('new value'));
