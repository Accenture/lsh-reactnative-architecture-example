...
class Cache {
  ...
  _has(filepath, field) {
    return (
      Object.prototype.hasOwnProperty.call(this._data, filepath) &&
      (!field ||
        Object.prototype.hasOwnProperty.call(
          this._data[filepath].data,
          field,
        ))
    );
  }
  ...
  _set(filepath, field, loaderPromise) {
    ...
    record.data[field] = loaderPromise
      .then(data =>
        Promise.all([data, Promise.denodeify(fs.stat)(filepath)]),
      )
      .then(([data, stat]) => {
  ...
  _getCacheFilePath(options) {
    ...
    var roots = options.projectRoots
      .join(',')
      .split(path.sep)
      .join('-');
    hash.update(roots);
