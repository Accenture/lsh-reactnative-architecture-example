...
describe('processRequest', () => {
  ...
  const makeRequest = (reqHandler, requrl) =>
    new Promise(resolve =>
      reqHandler(
        { url: requrl },
        {
  ...
  describe('file changes', () => {
    pit('invalides files in bundle when file is updated', () => {
      return makeRequest(
        ...
        expect(invalidatorFunc.mock.calls[0][0]).toEqual(
          'root/path/file.js',
        );
      });
    });
    ...
    pit(
      'rebuilds the bundles that contain a file when that file is changed',
      () => {
  ...
  describe('/assets endpoint', () => {
    ...
    it('should serve simple case', () => {
      ...
      AssetServer.prototype.get.mockImpl(() =>
  ...
  describe('buildBundleFromUrl(options)', () => {
    it('Calls the bundler with the correct args', () => {
      server.buildBundleFromUrl(
        '/path/to/foo.bundle?dev=false&runModule=false',
      );
      expect(Bundler.prototype.bundle).toBeCalledWith(
        'path/to/foo.js',
        false,
