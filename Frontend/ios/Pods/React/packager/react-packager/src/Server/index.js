...
class Server {
  constructor(options) {
    ...
    var watchRootConfigs = opts.projectRoots.map(dir => {
      return {
        dir: dir,
        globs: ['**/*.js', '**/*.json'].concat(assetGlobs),
      };
    });
    ...
    Promise.all([this._fileWatcher.end(), this._bundler.kill()]);
  }
  ...
  _rebuildBundles() {
    ...
    Object.keys(bundles).forEach(function(optionsJson) {
      ...
      bundles[optionsJson] = (
        bundles[optionsJson] || Promise.resolve()
      ).finally(function() {
  ...
  _processAssetsRequest(req, res) {
    ...
    const assetEvent = Activity.startEvent(
      `processing asset request ${assetPath[1]}`,
    );
    this._assetServer
  ...
  processRequest(req, res, next) {
    ...
    const building =
      this._bundles[optionsJson] || this.buildBundle(options);
  ...
  _getOptionsFromUrl(reqUrl) {
    ...
    return {
      ...
      runModule: this._getBoolOptionFromQuery(
        urlObj.query,
        'runModule',
        true,
      ),
