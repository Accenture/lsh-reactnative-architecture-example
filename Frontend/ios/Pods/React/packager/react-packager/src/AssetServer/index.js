...
class AssetServer {
  ...
  getAssetData(assetPath) {
    ...
    return this._getAssetRecord(assetPath)
      .then(record => {
        data.scales = record.scales;

        return Promise.all(record.files.map(file => stat(file)));
      })
      .then(stats => {
  ...
  _getAssetRecord(assetPath, platform = null) {
    ...
    return this._findRoot(this._roots, path.dirname(assetPath))
      ...
      .then(res => {
        ...
        if (platform != null) {
          record =
            map[getAssetKey(assetData.assetName, platform)] ||
