...
describe('Bundler', function() {
  ...
  beforeEach(function() {
    ...
    require('../../JSTransformer').prototype.loadFileAndTransform.mockImpl(
      function(path) {
        return Promise.resolve({
  ...
  pit('create a bundle', function() {
    return bundler
      .bundle('/root/foo.js', true, 'source_map_url')
      .then(function(p) {
        ...
        expect(p.addModule.mock.calls[3][0]).toEqual({
          code:
            'lol module.exports = require("AssetRegistry").registerAsset(' +
            JSON.stringify(imgModule) +
            '); lol',
          sourceCode:
            'module.exports = require("AssetRegistry").registerAsset(' +
            JSON.stringify(imgModule) +
            ');',
        ...
        expect(p.addAsset.mock.calls).toContain([imgModule_DEPRECATED]);

        expect(p.addAsset.mock.calls).toContain([imgModule]);
  ...
  pit('gets the list of dependencies from the resolver', function() {
    return bundler.getDependencies('/root/foo.js', true).then(() =>
      expect(getDependencies).toBeCalledWith('/root/foo.js', {
        dev: true,
      }),
    );
  });
