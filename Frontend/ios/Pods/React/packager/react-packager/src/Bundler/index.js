...
class Bundler {
    ...
    return this.getDependencies(main, isDev, platform)
      .then(response => {
        ...
        if (process.stdout.isTTY) {
          bar = new ProgressBar(
            'transforming [:bar] :percent :current/:total',
            {
              complete: '=',
              incomplete: ' ',
              width: 40,
        ...
        return Promise.all(
          response.dependencies.map(module =>
            this._transformModule(bundle, response, module, platform).then(
              transformed => {
                if (bar) {
                  bar.tick();
                }
  ...
  _transformModule(bundle, response, module, platform = null) {
    ...
    return transform.then(transformed =>
      resolver.wrapModule(response, module, transformed.code).then(
        code =>
          new ModuleTransport({
            code: code,
            map: transformed.map,
  ...
  generateAssetModule_DEPRECATED(bundle, module) {
    return Promise.all([sizeOf(module.path), module.getName()]).then(
      ([dimensions, id]) => {
        const img = {
