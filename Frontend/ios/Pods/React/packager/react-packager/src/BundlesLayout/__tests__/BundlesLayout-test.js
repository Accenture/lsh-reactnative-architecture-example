...
describe('BundlesLayout', () => {
  ...
  describe('generate', () => {
    ...
    pit(
      'separate bundle sync dependencies of async ones on same bundle',
      () => {
        DependencyResolver.prototype.getDependencies.mockImpl(path => {
          switch (path) {
            case '/root/index.js':
    ...
    pit('separate cache in which bundle is each dependency', () => {
      ...
      return layout.generateLayout(['/root/index.js']).then(() => {
        expect(layout.getBundleIDForModule('/root/index.js')).toBe(
          'bundle.0',
        );
        expect(layout.getBundleIDForModule('/root/a.js')).toBe('bundle.0');
        expect(layout.getBundleIDForModule('/root/b.js')).toBe(
          'bundle.0.1',
        );
        expect(layout.getBundleIDForModule('/root/c.js')).toBe(
          'bundle.0.2',
        );
        expect(layout.getBundleIDForModule('/root/d.js')).toBe(
          'bundle.0.1.3',
        );
      });
    });
