...
describe('BundlesLayout', () => {
  ...
  describe('generate', () => {
    ...
    function stripPolyfills(bundle) {
      return Promise.all(
        ...
        const modules = bundle.modules.filter(moduleName => {
          // filter polyfills
    ...
    pit('should bundle single-module app', () => {
      ...
      return newBundlesLayout()
        .generateLayout(['/root/index.js'])
        .then(bundles =>
    ...
    pit(
      'should split into multiple bundles separate async dependencies',
      () => {
        setMockFilesystem({
          root: {
            'index.js': `
    ...
    pit(
      'should include sync dependencies async dependencies might have',
      () => {
        setMockFilesystem({
          root: {
            'index.js': `
    ...
    pit(
      'should put in separate bundles async dependencies of async dependencies',
      () => {
        setMockFilesystem({
          root: {
            'index.js': `
  ...
  function getBaseFs() {
    const p = path
      .join(__dirname, '../../../DependencyResolver/polyfills')
      .substring(1);
    const root = {};
    ...
    polyfills.forEach(
      polyfill => (currentPath[polyfill.split('/')[1]] = ''),
    );
