...
function _scheduleAction(action) {
  ...
  if (_scheduledCollectionTimer === null) {
    _scheduledCollectionTimer = setTimeout(
      _runCollection,
      COLLECTION_PERIOD,
    );
  }
...
function _writeAction(action) {
  ...
  switch (action.action) {
    case 'startEvent':
      console.log(
        chalk.dim(
          '[' + fmtTime + '] ' + '<START> ' + action.eventName + data,
        ),
      );
      break;
    ...
    case 'endEvent':
      ...
      const startData = startAction.data
        ? ': ' + JSON.stringify(startAction.data)
        : '';
      console.log(
        chalk.dim(
          '[' +
            ...
            ' (' +
            (action.tstamp - startAction.tstamp) +
            'ms)' +
            startData,
        ),
      );
